...
def plot_param_history_over_iterations(
    ...
    traces = [
        (name, np.asarray([step_values[name] for step_values in param_history]))
        for name in fit_params
    ]
    plot_traces_grid(
        traces,
        suptitle="Gradient Descent Parameter Trajectories",
        xlabel="Iteration",
        save_path=save_path,
        show=show,
        bounds=param_bounds,
...
if __name__ == "__main__":
    ...
    plot_loss_over_iterations(
        ...
        title=f"Gradient Descent Loss Trajectory: {', '.join(config.fit_params)}",
